...
$conn = new PDO_wrapper($dsn, $username, $password, $options);

// 1. Injection SQL (Exécution de requêtes sans préparation - pour tester la vulnérabilité)
function getUserInfo(PDO_wrapper $conn, $userId) {
...
function getUserData(PDO_wrapper $conn, $userId) {
    $query = "SELECT * FROM users WHERE id = :userId";
    $params = ['userId' => $userId];
    $stmt = $conn->fetch($query, $params); // Requête préparée mais non validée
    return $stmt;
}
...
<div>
    <?= $_COOKIE['user'] ?> <!-- Cookie non sécurisé -->
</div>
